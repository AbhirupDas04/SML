...
def ComputeTree():
    ...
    decision_list = []
    mse_list = []
    residual_list = [i for i in labels]
    val_pred = [0 for i in range(val_len)]
    min_mse = 0
    best_iter = 0
    ...
    for i in range(300):
        ...
        for j in range(5):
            ...
            for k in range(1000):
                if len(sorted_class_list[j][k][0]) == n_left:
                    ...
                    n_left = len(sorted_class_list[j][k][0])
    ...
    test_pred = [0 for i in range(test_len)]
